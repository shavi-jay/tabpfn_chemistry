...
def run_active_learning_experiment(
    ...
    rng_random_method = np.random.default_rng(active_learning_seed + 1)  # Separate RNG for random method to ensure different randomness
    ...
    initial_train_size = min(initial_train_size, len(X_train))
    ...
    for i in tqdm(range(n_steps), desc="Active Learning Loop"):
        ...
        # Choose candidate ids from unseen ids
        candidate_ids = rng.choice(unseen_ids, size=min(num_candidates, len(unseen_ids)), replace=False)
        ...
        if uncertainty_method == "epistemic":
            unseen_total_uncertainty, unseen_aleatoric_uncertainty = vud_repeat_decomposition(
                ...
                test_features=X_train[candidate_ids],
            ...
            uncertainty_scores = rng_random_method.random(len(candidate_ids))
        else:
        ...
        new_seen_ids = candidate_ids[top_k_indices]
    ...
    result = {
        ...
        "new_train_ids": np.setdiff1d(train_ids, seen_ids).tolist()
    }
    results.append(result)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--initial_train_size", type=int, default=10, help="Initial number of training points to start active learning with")
    parser.add_argument("--num_candidates", type=int, default=20, help="Number of candidate points to consider for uncertainty sampling at each active learning step")
    parser.add_argument("--train_data_increment", type=int, default=10, help="Number of data points to add to training set at each active learning step")
    ...
    run_active_learning_experiment(
        ...
        train_data_increment=args.train_data_increment
    )